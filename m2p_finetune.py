...
from models_for_downstream import MultiModalEncoderDecoder
...
def loss_fn(logits, targets, num_classes):
    if num_classes == 1:
        loss_fct = nn.L1Loss()
        loss = loss_fct(logits.view(-1), targets.view(-1).float())
    else:
...
def run(args, config, train_data, valid_data, test_data=None):
    ...
    ########################### CREATE MODEL #################################
    model = MultiModalEncoderDecoder(ckpt_path=args.ckpt_path, num_classes=config['downstream']['label_num'])
    model.cuda()
    ...
    for epoch in range(epochs):
        ...
        for acoustic_inputs, semantic_inputs, label_inputs, _ in progress:
            ...
            loss = loss_fn(logits, label_inputs, num_classes=config['downstream']['label_num'])
...
if __name__ == '__main__':
    ...
    if args.task_name == 'emotion':
        ...
        data_root = '/share/kangyu/data/iemocap_mel160/'
        data_source = ['/share/kangyu/data/iemocap_mel160/Session1.pkl',
                       '/share/kangyu/data/iemocap_mel160/Session2.pkl',
                       '/share/kangyu/data/iemocap_mel160/Session3.pkl',
                       '/share/kangyu/data/iemocap_mel160/Session4.pkl',
                       '/share/kangyu/data/iemocap_mel160/Session5.pkl']
        ...
        data_root = '/share/kangyu/data/mel160/mel_160/'
        
        train_data = pickle.load(open('/share/kangyu/data/mel160/train.pkl','rb'))
        train_data = [(os.path.join(data_root,x[0]),x[1],x[2]) for x in train_data]
            
        valid_data = pickle.load(open('/share/kangyu/data/mel160/dev.pkl','rb'))
        valid_data = [(os.path.join(data_root,x[0]),x[1],x[2]) for x in valid_data]

        test_data = pickle.load(open('/share/kangyu/data/mel160/test.pkl','rb'))
        ...
        data_root = '/share/kangyu/data/libri_mel160'

        train_data = pickle.load(open('/share/kangyu/data/libri_mel160/verify_train.pkl','rb'))
        train_data = [(os.path.join(data_root,x[0]),os.path.join(data_root,x[1]),x[2]) for x in train_data]

        valid_data = pickle.load(open('/share/kangyu/data/libri_mel160/verify_dev.pkl','rb'))
        valid_data = [(os.path.join(data_root,x[0]),os.path.join(data_root,x[1]),x[2]) for x in valid_data]

        test_data = pickle.load(open('/share/kangyu/data/libri_mel160/verify_test.pkl','rb'))
